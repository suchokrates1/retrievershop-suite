...
from .services.product_taxonomy import edit_item_taxonomy_context, parse_product_form_taxonomy
...
def add_item():
    ...
    if form.validate_on_submit():
        try:
            category, brand, series = parse_product_form_taxonomy(request.form)
...
def edit_item(product_id):
    ...
    return render_template(
        ...
        **edit_item_taxonomy_context(product),
    )
...
def tiptop_reorder():
    ...
    if request.method == "POST":
        ...
        if action == "create_cart":
            ...
            return render_template(
                ...
                items=items,
                filler_script=script,
                tiptop_basket_url="https://tiptop24.pl/basket",
                tiptop_home_url="https://tiptop24.pl/",
            )

    with get_session() as db:
        lines = build_reorder_candidates(db)
        exclusions = list_exclusions(db)
    ...
    return render_template(
        ...
        threshold=getattr(app_settings, "LOW_STOCK_THRESHOLD", 5),
    )
...
def tiptop_cart_payload():
    from .services.tiptop_reorder import build_cart_payload, build_filler_script
    ...
    return jsonify(
        {
            "items": items,
            "script": build_filler_script(items),
            "basket_url": "https://tiptop24.pl/basket",
        }
    )
